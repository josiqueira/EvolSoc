...
V <- c(0.8,0.5) # [0,1] proporcao de sobrevivencia ate a idade reprodutiva
...
maxpopulacao <- 1
dt_historiatotal_populacao <- data.frame(matrix(nrow=ciclos, ncol=4))
...
for (ciclo in 0:ciclos)
{
  ...
  for (t.aux in 1:length(tipos))
  {
    dt_populacao$ocorre[dt_populacao$tipo==tipos[t.aux] & dt_populacao$prob<=probmorte.V[t.aux]] <- 1
  }
  ...
  dt_historiatotal_populacao$total[ciclo] <- total
  ...
  for (p.aux in 1:length(pops))
  {
    ...
    dt_historiatotal_populacao[ciclo, which(names(dt_historiatotal_populacao)==names(pops)[p.aux])] <- as.numeric(unlist(pops[p.aux]))
  }
...
openxlsx::write.xlsx(dt_historiatotal_populacao,"viabilidade_res.xlsx")
cat("\nTrajetorias guardadas em viabilidade_res.xlsx\n")
