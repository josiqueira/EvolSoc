...
V <- c(0.8,0.5) # [0,1] Vmax para (Vmax*(exp(-p)-exp(-1)))/(1-exp(-1))
...
for (ciclo in 0:ciclos)
{
  ...
  for (t.aux in 1:length(tipos))
  {
    ...
    viabilidade <- (V[t.aux]*(exp(-proporcao)-exp(-1)))/(1-exp(-1))
...
openxlsx::write.xlsx(dt_historiatotal_populacao,"viabilidadefncV_res.xlsx")
cat("\nTrajetorias guardadas em viabilidadefncV_res.xlsx\n")
